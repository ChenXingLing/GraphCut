	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				if (!next) { printf("¡¾Warning£¡£¡£¡¡¿Not enough memory!");/*if (error_function) (*error_function)("Not enough memory!");*/ exit(1); }
...
public:
	...
	Type *New()
	{
		...
		if (!first_free)
		{
			...
			if (!first) { printf("¡¾Warning£¡£¡£¡¡¿Not enough memory!");/*if (error_function) (*error_function)("Not enough memory!");*/ exit(1); }
